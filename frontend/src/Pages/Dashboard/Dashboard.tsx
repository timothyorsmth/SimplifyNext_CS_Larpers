...
import ArticleList from "../Articles/Articles.tsx";
...
function Dashboard() {
  ...
  const { appointments, careRecipient } = useCareRecipientInfo();
  const { effectiveTasks } = useTaskInfo();

  const defaultProfilePic = "/Assets/profile_pic_default.jpg";
  const pictureUrl =
    (loading ? defaultProfilePic : activeCaregiver?.profile_picture_url) ??
    defaultProfilePic;
  ...
  const todaysUpcoming = upcoming.filter(
    (a) => a.dateObj.toDateString() === nextDateKey,
  );
  ...
  return (
    <div className="Dashboard">
      ...
      <div className="WelcomeHeader">
        <div>
          <p>Good morning,</p>
          <h1 className="CaregiverName">
            {loading ? "" : `${activeCaregiver?.profile.first_name ?? "User"}.`}
          </h1>
        </div>
      ...
      <Link to="/schedule" className="UpcomingActivitiesCard">
        <div className="UpcomingActivitiesLabel">
          ...
          {nextDateKey && (
            <span className="UpcomingActivitiesDate">
              {formatDateHeader(new Date(nextDateKey))}
            </span>
          )}
        </div>
        <div className="UpcomingActivitiesList">
          {todaysUpcoming.length === 0 && (
            <div className="UpcomingActivityRow UpcomingActivityRow--empty">
              No upcoming activities
            </div>
          )}
          {todaysUpcoming.map((a) => (
            <div key={a.id} className="UpcomingActivityRow">
              <span className="UpcomingActivityTime">
                {formatTime(a.dateObj)}
              </span>
        ...
        <div className="YourTasksRow">
          {effectiveTasks.map((task) => {
            ...
            return (
              <div
                key={task.id}
                className={`DashboardTaskCard ${task.overdue ? "DashboardTaskCard--overdue" : "DashboardTaskCard--ontrack"}`}
              >
                <span className="DashboardTaskIcon">
                  {TaskIcon && <TaskIcon />}
                </span>
                <div>
      ...
      <ArticleList
        medicalHistory={careRecipient?.medicalHistory.map((entry) => entry.condition) ?? []}
      />
    </div>
  );
}
